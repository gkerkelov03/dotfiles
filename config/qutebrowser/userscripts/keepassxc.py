...
def parse_args():
    ...
    parser.add_argument(
        "--socket",
        "-s",
        default="/run/user/{}/org.keepassxc.KeePassXC.BrowserServer".format(
            os.getuid()
        ),
    ...
    parser.add_argument(
        "--insecure", action="store_true", help="Do not encrypt auth key"
    )
...
class KeepassXC:
    ...
    def connect(self):
        ...
        self.client_id = base64.b64encode(
        ...
        self.send_raw_msg(
            dict(
                ...
                publicKey=base64.b64encode(self.client_key.public_key.encode()).decode(
                    "utf-8"
                ),
        ...
        self.cryptobox = nacl.public.Box(
            self.client_key, nacl.public.PublicKey(base64.b64decode(resp["publicKey"]))
        )
    ...
    def associate(self):
        self.send_msg(
            dict(
                action="associate",
                key=base64.b64encode(self.client_key.public_key.encode()).decode(
                    "utf-8"
                ),
        ...
        self.send_msg(
            dict(
                ...
                keys=[
                    {
                        "id": self.id,
                        "key": base64.b64encode(self.id_key.public_key.encode()).decode(
                            "utf-8"
                        ),
                    }
                ],
    ...
    def get_totp(self, uuid):
        self.send_msg(dict(action="get-totp", uuid=uuid))
        ...
        self.send_raw_msg(
            dict(
                ...
                message=base64.b64encode(
                    self.cryptobox.encrypt(
                        json.dumps(msg).encode("utf-8"), nonce
                    ).ciphertext
                ).decode("utf-8"),
    ...
    def recv_msg(self):
        ...
        assert resp["action"]
        return json.loads(
            self.cryptobox.decrypt(
                base64.b64decode(resp["message"]), base64.b64decode(resp["nonce"])
            ).decode("utf-8")
        )
...
class SecretKeyStore:
    ...
    def load(self):
        ...
        if self.insecure:
            ...
            jsondata = subprocess.check_output(["gpg", "--decrypt", self.path]).decode(
                "utf-8"
            )
    ...
    def store(self, id):
        ...
        self.id = id
        jsondata = json.dumps(
            {"id": self.id, "key": base64.b64encode(self.key).decode("utf-8")}
        )
        if self.insecure:
            open(self.path, "w").write(jsondata)
        else:
            subprocess.run(
                ["gpg", "--encrypt", "-o", self.path, "-r", self.gpgkey],
                input=jsondata.encode("utf-8"),
                check=True,
            )
    ...
    try:
        ...
        idx = subprocess.check_output(
            [
                ...
                "Search",
                "-mesg",
                "<b>qute-keepassxc</b>: select an account, please!",
            ],
...
def main():
    if "QUTE_FIFO" not in os.environ:
        print(
            f"No QUTE_FIFO found - {sys.argv[0]} must be run as a qutebrowser userscript"
        )
        sys.exit(-1)
